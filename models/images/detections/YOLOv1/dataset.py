...
class YOLOv1DataLoader(Sequence):
    def __init__(self, image_dir, label_dir, image_size,
        ...
        self.image_paths = [os.path.join(image_dir, f) for f in os.listdir(image_dir) if f.endswith('.jpg') or f.endswith('.png') or f.endswith('.ppm')]
        self.label_paths = [p.replace('images', 'labels').replace('.jpg', '.txt').replace('.png', '.txt').replace('.ppm', '.txt')  for p in self.image_paths]
...
if __name__ == '__main__':
    ...
    from utils import yolo2xywh, plot_labels, LABELS_DICT_CATEGORIZED_GTSRB
    
    i = 5
    plot_labels(images[i], yolo2xywh(labels[i]), class_labels=LABELS_DICT_CATEGORIZED_GTSRB, num_grids=7, relative_to_grids=True)
    plt.show()
    
    from loss import YOLO_2bboxes
    import jax.numpy as jnp
    y_pred = jnp.concatenate([labels[..., :5], labels], axis=-1)
    loss0 = YOLO_2bboxes()(y_pred, labels)
    
    y_pred = jnp.concatenate([labels[..., 0:1], labels[..., 1:5]+5, labels[..., :5]+1, labels[..., 5:]+1], axis=-1)
    loss1 = YOLO_2bboxes()(y_pred, labels)
